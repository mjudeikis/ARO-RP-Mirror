...
var _ = Describe("Cluster smoke test", func() {
	...
	var _ = BeforeEach(func() {
		...
		err := project.Create(context.TODO())
		Expect(err).NotTo(HaveOccurred(), "Failed to create test namespace")

		Eventually(func() error {
			return project.Verify(context.TODO())
		}).Should(BeNil())
	})

	var _ = AfterEach(func() {
		err := project.Delete(context.TODO())
		Expect(err).NotTo(HaveOccurred(), "Failed to delete test namespace")

		Eventually(func() error {
			return project.VerifyProjectIsDeleted(context.TODO())
	...
	Specify("Can create load balancer services", func() {
		...
		Eventually(func() bool {
			svc, err := clients.Kubernetes.CoreV1().Services(testNamespace).Get(context.TODO(), "elb", metav1.GetOptions{})
			if err != nil {
				return false
			}
		...
		Eventually(func() bool {
			svc, err := clients.Kubernetes.CoreV1().Services(testNamespace).Get(context.TODO(), "ilb", metav1.GetOptions{})
			if err != nil {
				return false
			}
			return ready.ServiceIsReady(svc)
...
func createPVC(ctx context.Context, cli kubernetes.Interface) error {
	...
	_, err = cli.CoreV1().PersistentVolumeClaims(testNamespace).Create(context.TODO(), &corev1.PersistentVolumeClaim{
...
func createPodWithPVC(ctx context.Context, cli kubernetes.Interface) error {
	...
	_, err := cli.CoreV1().Pods(testNamespace).Create(context.TODO(), &corev1.Pod{
		ObjectMeta: metav1.ObjectMeta{
			Name: testPodName,
		},
		Spec: corev1.PodSpec{
...
func verifyPodSucceeded(cli kubernetes.Interface) error {
	pod, err := cli.CoreV1().Pods(testNamespace).Get(context.TODO(), testPodName, metav1.GetOptions{})
	if err != nil {
		return err
	}
...
func createLoadBalancerService(ctx context.Context, cli kubernetes.Interface, name string, annotations map[string]string) error {
	...
	_, err := cli.CoreV1().Services(testNamespace).Create(context.TODO(), svc, metav1.CreateOptions{})
	return err
}
