...
var _ = Describe("[Admin API] Kubernetes objects action", func() {
	...
	When("in a standard openshift namespace", func() {
		...
		It("should be able to create, get, list, update and delete objects", func() {
			defer func() {
				...
				err := clients.Kubernetes.CoreV1().ConfigMaps(namespace).Delete(context.TODO(), objName, metav1.DeleteOptions{})
	...
	When("in a customer namespace", func() {
		...
		It("should be able to get and list existing objects, but not update and delete or create new objects", func() {
			...
			_, err := clients.Kubernetes.CoreV1().Namespaces().Create(context.TODO(), &corev1.Namespace{
			...
			defer func() {
				...
				err := clients.Kubernetes.CoreV1().Namespaces().Delete(context.TODO(), namespace, metav1.DeleteOptions{})
				Expect(err).NotTo(HaveOccurred())
				...
				err = wait.PollImmediate(10*time.Second, 5*time.Minute, func() (bool, error) {
					_, err := clients.Kubernetes.CoreV1().Namespaces().Get(context.TODO(), namespace, metav1.GetOptions{})
					if err == nil {
						return false, nil
					}
					if !errors.IsNotFound(err) {
			...
			_, err = clients.Kubernetes.CoreV1().ConfigMaps(namespace).Create(context.TODO(), &corev1.ConfigMap{
...
func testConfigMapCreateOK(resourceID, objName, namespace string) {
	...
	cm, err := clients.Kubernetes.CoreV1().ConfigMaps(namespace).Get(context.TODO(), objName, metav1.GetOptions{})
	Expect(err).NotTo(HaveOccurred())
...
func testConfigMapDeleteOK(resourceID, objName, namespace string) {
	...
	err = wait.PollImmediate(10*time.Second, time.Minute, func() (bool, error) {
		_, err = clients.Kubernetes.CoreV1().ConfigMaps(namespace).Get(context.TODO(), objName, metav1.GetOptions{})
		if err == nil {
			return false, nil
		}
		if !errors.IsNotFound(err) {
