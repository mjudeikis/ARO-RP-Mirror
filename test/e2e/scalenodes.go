...
var _ = Describe("Scale nodes", func() {
	...
	Specify("node count should match the cluster resource and nodes should be ready", func() {
		...
		err = wait.PollImmediate(10*time.Second, 30*time.Minute, func() (bool, error) {
			nodes, err := clients.Kubernetes.CoreV1().Nodes().List(context.TODO(), metav1.ListOptions{})
			if err != nil {
				return false, err
			}
	...
	Specify("nodes should scale up and down", func() {
		mss, err := clients.MachineAPI.MachineV1beta1().MachineSets("openshift-machine-api").List(context.TODO(), metav1.ListOptions{})
		Expect(err).NotTo(HaveOccurred())
...
func scale(name string, delta int32) error {
	return retry.RetryOnConflict(retry.DefaultRetry, func() error {
		ms, err := clients.MachineAPI.MachineV1beta1().MachineSets("openshift-machine-api").Get(context.TODO(), name, metav1.GetOptions{})
		if err != nil {
			return err
		}
		...
		*ms.Spec.Replicas += delta
		...
		_, err = clients.MachineAPI.MachineV1beta1().MachineSets(ms.Namespace).Update(context.TODO(), ms, metav1.UpdateOptions{})
		return err
	})
}
