...
func (sr *systemreserved) vmSize() (string, error) {
	...
	for _, machineset := range machinesets.Items {
		...
		if largestVMSize == "" {
			largestVMSize = machineProviderSpec.VMSize
		} else {
			largestMEMSize, err := vmSizeToMemorySize(largestVMSize)
			if err != nil {
				return "", err
			}

			memSize, err := vmSizeToMemorySize(machineProviderSpec.VMSize)
			if err != nil {
				return "", err
			}
	...
	return largestVMSize, nil
}

func vmSizeToMemorySize(vmSize string) (int, error) {
	cap, ok := vmCapacity[vmSize]
	if !ok {
		return 0, fmt.Errorf("VMSize %s not supported", vmSize)
	}

	return cap["MemGB"], nil
}
