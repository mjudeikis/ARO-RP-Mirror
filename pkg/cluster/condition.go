...
func (i *manager) bootstrapConfigMapReady(ctx context.Context) (bool, error) {
	cm, err := i.kubernetescli.CoreV1().ConfigMaps("kube-system").Get(ctx, "bootstrap", metav1.GetOptions{})
	return err == nil && cm.Data["status"] == "complete", nil
}

func (i *manager) apiServersReady(ctx context.Context) (bool, error) {
	apiserver, err := i.configcli.ConfigV1().ClusterOperators().Get(ctx, "kube-apiserver", metav1.GetOptions{})
	if err != nil {
		return false, nil
	}
...
func (i *manager) operatorConsoleExists(ctx context.Context) (bool, error) {
	_, err := i.operatorcli.OperatorV1().Consoles().Get(ctx, consoleapi.ConfigResourceName, metav1.GetOptions{})
	return err == nil, nil
}

func (i *manager) operatorConsoleReady(ctx context.Context) (bool, error) {
	consoleOperator, err := i.configcli.ConfigV1().ClusterOperators().Get(ctx, "console", metav1.GetOptions{})
	if err != nil {
		return false, nil
	}
...
func (i *manager) ingressControllerReady(ctx context.Context) (bool, error) {
	ingressOperator, err := i.configcli.ConfigV1().ClusterOperators().Get(ctx, "ingress", metav1.GetOptions{})
	if err != nil {
		return false, nil
	}
