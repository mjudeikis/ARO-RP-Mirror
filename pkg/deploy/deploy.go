...
func (d *deployer) Deploy(ctx context.Context) error {
	...
	parameters.Parameters["adminApiCaBundle"] = &arm.ParametersParameter{
		Value: base64.StdEncoding.EncodeToString([]byte(to.String(d.config.Configuration.AdminAPICABundle))),
	}
	parameters.Parameters["domainName"] = &arm.ParametersParameter{
		Value: d.config.Location + "." + to.String(d.config.Configuration.ClusterParentDomainName),
	}
	...
	parameters.Parameters["rpImage"] = &arm.ParametersParameter{
		Value: to.String(d.config.Configuration.RPImagePrefix) + ":" + d.version,
	}
...
func (d *deployer) configureDNS(ctx context.Context) error {
	...
	zone, err := d.zones.Get(ctx, d.config.ResourceGroupName, d.config.Location+"."+to.String(d.config.Configuration.ClusterParentDomainName))
	if err != nil {
		return err
	}

	_, err = d.globalrecordsets.CreateOrUpdate(ctx, to.String(d.config.Configuration.GlobalResourceGroupName), to.String(d.config.Configuration.RPParentDomainName), "rp."+d.config.Location, mgmtdns.A, mgmtdns.RecordSet{
	...
	_, err = d.globalrecordsets.CreateOrUpdate(ctx, to.String(d.config.Configuration.GlobalResourceGroupName), to.String(d.config.Configuration.ClusterParentDomainName), d.config.Location, mgmtdns.NS, mgmtdns.RecordSet{
		RecordSetProperties: &mgmtdns.RecordSetProperties{
			TTL:       to.Int64Ptr(3600),
			NsRecords: &nsRecords,
		},
...
func (d *deployer) getParameters(ps map[string]interface{}) *arm.Parameters {
	...
	for i := 0; i < v.NumField(); i++ {
		...
		// arm.Parameters expect string
		// the Configuration has *string and have to be converted
		pointerToConfigOption, ok := v.Field(i).Interface().(*string)
		if ok {
			m[strings.SplitN(v.Type().Field(i).Tag.Get("json"), ",", 2)[0]] = *pointerToConfigOption
			continue
		}

		// some of the fields are not of the *string type but []interface{} type, so this line is needed
