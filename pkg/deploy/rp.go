...
func (g *generator) halfPeering(vnetA string, vnetB string) *arm.Resource {
	return &arm.Resource{
		Resource: &mgmtnetwork.VirtualNetworkPeering{
			...
			Name: to.StringPtr(fmt.Sprintf("'%s/%s-%s-peering'", vnetA, vnetA, vnetB)),
		},
...
func (g *generator) template() *arm.Template {
	...
	t.Resources = append(t.Resources, g.zone(), g.clustersKeyvault(), g.serviceKeyvault(), g.rpvnet(), g.pevnet())
	t.Resources = append(t.Resources, g.halfPeering("rp-vnet", "rp-pe-vnet-001"), g.halfPeering("rp-pe-vnet-001", "rp-vnet"))
	if g.production {
